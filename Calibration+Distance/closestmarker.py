...
calib_data_path = r"calib_data\\MultiMatrix.npz"
    ...
    while True:
        ...
        if marker_corners:
            ...
            for ids, corners, i in zip(marker_IDs, marker_corners, total_markers):
                ...
                distance=distance-5 #error correction for current camera calibratioon
                dictmarkers[ids[0]] = [distance,corners]
                temp = min(dictmarkers.values())
                res = [key for key in dictmarkers if dictmarkers[key] == temp]
                cv.polylines(
                    frame, [dictmarkers[res[0]][1].astype(
                ...
                for markers in dictmarkers:
                    if markers==res: continue
                    cv.polylines(
                        frame, [dictmarkers[markers][1].astype(
